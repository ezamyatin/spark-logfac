...
class OptimizerSuite extends MLTest with Logging {
  ...
  test("Optimizer explicit") {
    ...
    logInfo(f"True test accuracy is $trueAcc.")
    logInfo(f"Random test accuracy is $rndAcc.")
    logInfo(f"Actual test accuracy is $acc.")
  ...
  test("Optimizer implicit") {
    ...
    logInfo(f"True test epr is $trueEpr.")
    logInfo(f"Random test epr is $rndEpr.")
    logInfo(f"Actual test epr is $epr.")
...
object OptimizerSuite extends Logging {
  ...
  private[recommendation] def genData(
    ...
    (userFactors, itemFactors,
      trainData.groupBy(e => (e._1, e._2, e._3)).mapValues(_.map(_._4).sum)
        .map(e => (e._1._1, e._1._2, e._1._3, e._2)).toArray,
      testData.groupBy(e => (e._1, e._2, e._3)).mapValues(_.map(_._4).sum)
        .map(e => (e._1._1, e._1._2, e._1._3, e._2)).toArray)
  }
}
