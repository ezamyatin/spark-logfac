...
private[ml] class Optimizer(private val opts: Opts,
  ...
  private def remap(pair: LongPairMulti, inplace: Boolean): LongPairMulti = {
    ...
    newPair.left.indices.foreach{i =>
      newPair.left(i) = if (vocabL.contains(newPair.left(i))) vocabL(newPair.left(i)).toLong else -1L
      newPair.right(i) = if (vocabR.contains(newPair.right(i))) vocabR(newPair.right(i)).toLong else -1L
    }
